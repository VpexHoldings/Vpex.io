...
contract VPEX is Context, IERC20, Ownable {
    ...
    string private _name = "VPEX";
    string private _symbol = "VPX";
    ...
    uint256 private numTokensSellToAddToLiquidity = 2 * 10**7 * 10**18;

    // listing restrictions
    uint256 private _tradingTime;
    uint256 private _restrictionLiftTime;
    uint256 private _maxRestrictionAmount = 2 * 10**6 * 10**9;  // maximum trade amount allowed in the first 10 minutes
...
// This make it secure from bots
    ...
    modifier launchRestrict(address sender, address recipient, uint256 amount) {
        if (_tradingTime > now) {
            require(sender == owner() || recipient == owner(), "VPEX: transfers are disabled");
        } else if (_tradingTime <= now && _restrictionLiftTime > now) {
            require(amount <= _maxRestrictionAmount, "VPEX: amount greater than max limit in restricted mode");
            if (!_isWhitelisted[sender] && !_isWhitelisted[recipient]) {
                require(_lastTx[sender].add(60) <= now && _lastTx[recipient].add(60) <= now, "VPEX: only one tx/min in restricted mode");
                _lastTx[sender] = now;
                _lastTx[recipient] = now;
            } else if (!_isWhitelisted[recipient]){
                require(_lastTx[recipient].add(60) <= now, "VPEX: only one tx/min in restricted mode");
                _lastTx[recipient] = now;
            } else if (!_isWhitelisted[sender]) {
                require(_lastTx[sender].add(60) <= now, "VPEX: only one tx/min in restricted mode");
                _lastTx[sender] = now;
            }
        }
        _;
...
// Bot End here
    ...
    function _transfer(
        ...
        require(!_paused || from == owner() || to == owner(), "VPEX: Transfers are paused"); // pause check
